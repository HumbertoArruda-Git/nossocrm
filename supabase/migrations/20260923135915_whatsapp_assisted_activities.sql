-- Assisted WhatsApp records remain ordinary CRM activities.
...
BEGIN
  ...
  IF p_event = 'follow_up_sent' THEN
    SELECT id INTO v_task_id FROM public.activities
    ...
    END IF;
    UPDATE public.activities SET completed = true WHERE id = v_task_id;
  END IF;
  ...
  VALUES
    (v_org_id, p_deal_id, v_deal.contact_id, auth.uid(), 'NOTE',
     CASE p_event WHEN 'initial_sent' THEN 'WhatsApp inicial confirmado'
       WHEN 'follow_up_sent' THEN 'Follow-up WhatsApp confirmado'
  ...
  IF p_event = 'initial_sent' THEN
    ...
    VALUES
      (v_org_id, p_deal_id, v_deal.contact_id, auth.uid(), 'TASK',
       'Follow-up WhatsApp', 'Retomar contato pelo WhatsApp.',
       (v_due_date + time '09:00') AT TIME ZONE 'America/Sao_Paulo',
       false, jsonb_build_object('channel', 'whatsapp', 'event', 'follow_up_due',
         'status', 'pending', 'source_activity_id', v_activity_id::text))
    RETURNING id INTO v_task_id;
  ELSIF p_event = 'replied' THEN
    UPDATE public.activities SET completed = true
    WHERE deal_id = p_deal_id AND organization_id = v_org_id
  ...
  RETURN jsonb_build_object('activity_id', v_activity_id,
    ...
    'task_id', v_task_id, 'duplicate', false);
END;
$$;
