-- Assisted WhatsApp: Prospector deals arrive without a contact or phone. On a confirmed send
-- the typed phone is persisted in the same transaction as the activity: the deal's contact
-- gets the phone if it has none, or a contact is created and linked to the deal.
-- The new p_phone parameter changes the signature, so the 5-argument version is replaced
-- (keeping both would make named RPC calls ambiguous). Drop + create run in one transaction.
...
BEGIN
  ...
  END IF;
  -- The API validates the number (libphonenumber); here only its stored shape: E.164.
  IF p_phone IS NOT NULL AND p_phone !~ '^\+[1-9][0-9]{9,14}$' THEN
  ...
  FOR UPDATE;
  IF NOT FOUND OR v_deal.is_won OR v_deal.is_lost THEN
  ...
  IF v_deal.contact_id IS NOT NULL THEN
    SELECT organization_id, phone INTO v_contact_org_id, v_contact_phone
  ...
  -- A retry of a confirmation that already went through returns it untouched: no second
  -- contact, activity or task.
  ...
  IF v_existing_id IS NOT NULL THEN
    RETURN jsonb_build_object('activity_id', v_existing_id, 'stage_id', v_deal.stage_id,
      'contact_id', v_deal.contact_id, 'duplicate', true);
  END IF;
  ...
  -- Contact and phone, persisted only now that the send was confirmed.
  IF v_deal.contact_id IS NULL THEN
    ...
    END IF;
    -- One contact per deal. Linking another deal's contact by phone is avoided on purpose:
    -- check_deal_duplicate forbids two open deals of one contact in the same stage.
    SELECT name INTO v_company_name FROM public.crm_companies
    WHERE id = v_deal.client_company_id AND organization_id = v_org_id AND deleted_at IS NULL;
    -- No real person name is known; like the WhatsApp webhook, the number names the contact.
    INSERT INTO public.contacts
      (organization_id, owner_id, name, phone, company_name, client_company_id, source, status, stage)
    VALUES
      (v_org_id, auth.uid(), p_phone, p_phone, v_company_name,
       CASE WHEN v_company_name IS NOT NULL THEN v_deal.client_company_id END,
       'whatsapp', 'ACTIVE', 'LEAD')
    RETURNING id INTO v_deal.contact_id;
    v_contact_created := true;
    UPDATE public.deals SET contact_id = v_deal.contact_id, updated_at = v_now
    WHERE id = p_deal_id AND organization_id = v_org_id;
  ELSIF p_phone IS NOT NULL AND p_event <> 'replied'
     AND length(regexp_replace(coalesce(v_contact_phone, ''), '\D', '', 'g')) < 10 THEN
    -- The deal's own contact had no usable number: fill it, never overwrite a real one.
    UPDATE public.contacts SET phone = p_phone, updated_at = v_now
    WHERE id = v_deal.contact_id AND organization_id = v_org_id;
  END IF;
  ...
  RETURN jsonb_build_object('activity_id', v_activity_id,
    ...
    'task_id', v_task_id, 'follow_up_number', v_follow_up_number,
    'contact_id', v_deal.contact_id, 'contact_created', v_contact_created, 'duplicate', false);
END;
$$;

REVOKE ALL ON FUNCTION public.record_assisted_whatsapp(uuid, text, text, uuid, uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.record_assisted_whatsapp(uuid, text, text, uuid, uuid, text) TO authenticated;
