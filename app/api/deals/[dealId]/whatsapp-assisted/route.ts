...
const bodySchema = z.object({
  ...
  /** Number the user opened wa.me with; persisted only by this confirmation. */
...
export async function POST(
  ...
  // Replies never touch the phone; sends only persist a number that passes the same check as wa.me.
  let phone: string | null = null;
  if (body.event !== 'replied' && body.phone?.trim()) {
    const digits = validatedWhatsAppPhone(body.phone);
    if (!digits) {
  ...
  if ((deal.contact_id && !contactResult.data) || boardResult.data?.key !== 'prospeccao-comercial') {
    return NextResponse.json({ error: 'Contato ou quadro indisponível' }, { status: 404 });
  }
  if (!deal.contact_id && body.event !== 'replied' && !phone) {
